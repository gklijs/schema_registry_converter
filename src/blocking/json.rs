...
fn to_bytes(id: u32, value: &Value) -> Vec<u8> {
    get_payload(id, serde_json::to_vec(value).unwrap())
}
...
fn add_refs_to_scope(
    ...
    for rr in refs.iter() {
        ...
        let def: Value = match serde_json::from_str(&*rs.schema) {
            Ok(v) => v,
            Err(e) => {
...
fn fetch_id(def: &Value) -> Option<Url> {
    let id = match def {
...
fn fetch_fallback(url: &str, id: u32) -> Url {
...
mod tests {
    ...
    fn test_encode_clean_cache() {
        ...
        let encoded_data = encoder.encode(result_example.clone(), &strategy);
    ...
    fn add_referred_schema() {
        ...
        let result = match decoder.decode(Some(&bytes)) {
            Ok(Some(v)) => v.value,
            _ => panic!("Not a value while that was expeced"),
        };
        match result {
            Value::Object(v) => {
                assert_eq!(&Value::String(String::from("Java")), v.get("by").unwrap())
            }
            _ => panic!("Not an Object that was expeced"),
        }
    }

    #[test]
    fn error_in_refered_schema() {
        let sr_settings = SrSettings::new(format!("http://{}", server_address()));
        let mut decoder = JsonDecoder::new(sr_settings);
        let bytes = [
            0, 0, 0, 0, 5, 123, 34, 105, 100, 34, 58, 91, 52, 53, 44, 45, 55, 57, 44, 57, 51, 44,
